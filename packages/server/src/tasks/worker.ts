...
export const addPipeline: AddJobFn<typeof taskList> = async (
  taskName,
  payload
) => {
...
async function main() {
  ...
  const getTaskOrPipeline = async (jobId: string) => {
  ...
  const setTaskOrPipelineStatus = async (jobId: string, status: TaskStatus) => {
    const type = await getTaskOrPipeline(jobId);
    if (type === "pipeline") {
  ...
  runner.events.on("job:error", (err) => {
    setTaskOrPipelineStatus(err.job.id, "retrying");
  });

  runner.events.on("job:success", ({ job }) => {
    setTaskOrPipelineStatus(job.id, "completed");
  });

  runner.events.on("job:complete", ({ job, error }) => {
    if (error) {
      setTaskOrPipelineStatus(job.id, "failed");
    } else {
      setTaskOrPipelineStatus(job.id, "completed");
    }
  });
...
(async () => {
  ...
  const job = await addPipeline("testSaga", { name: "Jamesb" });

  const updatedJobs = (await workerUtils()).permanentlyFailJobs(
    [],
    "Enter reason here"
  );
})();
